...
const CALLS_OPTIONS = ["Fewer than 5", "5–15", "15–30", "30+"];
...
export function PilotForm() {
  ...
  if (submitted) {
    return (
      <div className="rounded-xl border border-line bg-paper p-10 text-center">
        ...
        <p className="mt-3 max-w-md mx-auto text-sm leading-relaxed text-ink-soft">
          We&rsquo;ll review your property information and follow up shortly. If you&rsquo;d like to move faster, book a coverage review now and we&rsquo;ll come prepared.
        </p>
        <div className="mt-7">
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const afterHours = get("afterHours");
    ...
    try {
      ...
      trackEvent("pilot_qualification_submit", {
        ...
        after_hours_needed: afterHours || "unknown",
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-7">
      ...
      {/* Property details */}
      <div>
        <p className="mb-5 text-xs font-semibold uppercase tracking-[0.16em] text-ink-muted">About your property</p>
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div>
            <Label htmlFor="propertyWebsite">Property website</Label>
            <Input id="propertyWebsite" name="propertyWebsite" placeholder="yourhhotel.com" className="placeholder:text-ink-muted/50" />
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2">
          <div>
            ...
            <Input id="location" name="location" placeholder="Miami, FL" className="placeholder:text-ink-muted/50" />
          </div>
          <div>
            <Label htmlFor="callsPerDay">Estimated calls per day</Label>
            <Select id="callsPerDay" name="callsPerDay" defaultValue="">
              <option value="" disabled>Select range</option>
              {CALLS_OPTIONS.map((opt) => (
                <option key={opt} value={opt}>{opt}</option>
              ))}
            </Select>
          </div>
      ...
      {/* Coverage needs */}
      <div>
        <p className="mb-5 text-xs font-semibold uppercase tracking-[0.16em] text-ink-muted">Coverage needs</p>
        <div>
        ...
        <div className="mt-5">
          <Label>Do you need after-hours coverage?</Label>
          <div className="mt-3 flex flex-wrap gap-3">
            {["Yes", "No", "Not sure"].map((opt) => (
              <label key={opt} className="flex cursor-pointer items-center gap-2.5">
                <input type="radio" name="afterHours" value={opt} className="accent-[#C7A24A]" />
        ...
        <div className="mt-5">
          ...
          <Input
            id="tools"
            name="tools"
            placeholder="e.g. Cloudbeds, Akia, Booking.com, WhatsApp"
      ...
      {/* Contact */}
      <div>
        <p className="mb-5 text-xs font-semibold uppercase tracking-[0.16em] text-ink-muted">Your contact details</p>
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
      ...
      <div>
        ...
        <p className="mt-3 text-xs text-ink-muted">
          We review all submissions within one business day. Your information is confidential and never shared.
        </p>
      </div>
    </form>
  );
